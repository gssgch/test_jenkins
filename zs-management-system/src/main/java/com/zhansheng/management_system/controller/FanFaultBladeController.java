...
/**
 * @author xufu
 * @description 故障中心（叶片故障）（风机故障表处理）
 * @date 2019/9/30 10:28
 */

@RestController
@RequestMapping("/FanFaultBlade")
public class FanFaultBladeController implements FanFaultControllerApi {
    ...
    @Override
    @PreAuthorize("hasAuthority('blade_management')")
    @LogAnnotation
    @ApiOperation(value = "添加故障中心（叶片故障）")
    ...
    @PreAuthorize("hasAuthority('blade_management')")
    @LogAnnotation
    @ApiOperation(value = "删除故障中心（叶片故障）")
    ...
    @PreAuthorize("hasAuthority('blade_management')")
    @LogAnnotation
    @ApiOperation(value = "修改故障中心（叶片故障）")
