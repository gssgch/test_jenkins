...
/**
 * @author xufu
 * @description 故障中心（实时预警故障）（风机故障表处理）
 * @date 2019/9/30 10:28
 */

@RestController
@RequestMapping("/FanFault")
public class FanFaultController implements FanFaultControllerApi {
    ...
    @Override
    @PreAuthorize("hasAuthority('realtime_management')")
    @LogAnnotation
    @ApiOperation(value = "添加故障中心（实时预警故障）")
    ...
    @PreAuthorize("hasAuthority('realtime_management')")
    @LogAnnotation
    @ApiOperation(value = "删除故障中心（实时预警故障）")
    ...
    @PreAuthorize("hasAuthority('realtime_management')")
    @LogAnnotation
    @ApiOperation(value = "修改故障中心（实时预警故障）")
