...
@Service
@Transactional
public class TbMonitorServiceImpl implements TbMonitorInterface {
        ...
        try {
            for (TbLoop tbLoop : list) {
                ...
                for (TbDraughtfan draughtfan : idList) {
                    ...
                    TbState states = baseService.monitorStateOne(draughtNumber);
                    if (states == null) {
                        states = new TbState();
                        states.setF(0);
                    }
    ...
    public List<TbParticulars> findNumberListNew1(DraListDto draListDto) {
        ...
        fieldsObject.put("fNaceVibX", true);
        fieldsObject.put("fNaceVibY", true);
        fieldsObject.put("fTPitMotorVisu1", true);
        fieldsObject.put("fTPitMotorVisu2", true);
        fieldsObject.put("fTPitMotorVisu3", true);
        fieldsObject.put("fPitDrvI1", true);
        fieldsObject.put("fPitDrvI2", true);
        fieldsObject.put("fPitDrvI3", true);
        fieldsObject.put("fVibEffecVal", true);
    ...
    public Result findBydraughtNumberListNew(int draughtNumber) {
        ...
        if (draughtNumber > 0) {
            ...
            List<TbParticulars> numberList = findBydraughtNumberParticularsListNew(draughtNumber);
    ...
    private List<TbParticulars> findBydraughtNumberParticularsListNew(int draughtNumber) {
        ...
        BasicDBObject dbObject = new BasicDBObject();
        Query query = new BasicQuery(dbObject.toString(), fieldsObject.toString())
                .addCriteria(Criteria.where("fDraughtNumber").is(draughtNumber)).limit(100);
        query.with(new Sort(new Sort.Order(Sort.Direction.DESC, "fDateAcqTime")));
        return mongoTemplate.find(query, TbParticulars.class);
    }
 ...
 */
    ...
    public Result findBydraughtNumber(int draughtNumber) {
        Map<Object, Object> map = new HashMap<>();
        if (draughtNumber > 0) {
            List<TbState> bydraughtNumber = baseService.monitorStateLimit(draughtNumber);
