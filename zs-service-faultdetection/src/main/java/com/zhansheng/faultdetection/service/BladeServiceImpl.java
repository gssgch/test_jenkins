...
@Service
public class BladeServiceImpl implements BladeInterface {
            ...
            for (TbLoop tbLoop : list) {
                ...
                for (TbDraughtfan draught : idList) {
                    ...
                    for (TbState state : stateList) {
                        //状态
                        Integer fState1 = state.getFState2();
                        //叶片结冰状态
                        if (fState1 == 0) {
                            state_0++;
                        }
                ...
                for (TbDraughtfan draughtfan : idList) {
                    ...
                    TbState states = baseService.bladeList(draughtNumber);
                    if (states == null) {
                        states = new TbState();
                        states.setF(0);
                    }
    ...
    public Result findPageListNew(CommonList commonList) {
        ...
        for (TbLoop tbLoop : list) {
            ...
            for (TbDraughtfan draught : idList) {
                ...
                for (TbState state : stateList) {
                    //状态
                    Integer fState2 = state.getFState2();//叶片结冰
                    //叶片结冰状态
                    if (fState2 == -1) {//故障
                        state_0++;
                        draught.setState(-1);
    ...
    private List<TbParticulars> findBydraughtNumberParticularsListNew(int draughtNumber) {
        ...
        fieldsObject.put("fTOutd10m", true);
        fieldsObject.put("fTHubVisu", true);
        ...
        Query query = new BasicQuery(dbObject.toString(), fieldsObject.toString())
                .addCriteria(Criteria.where("fDraughtNumber").is(draughtNumber)).limit(200);
 ...
 */
    /*public Result findBydraughtNumber(int draughtNumber) {
        Map<Object, Object> map = new HashMap<>();
        if (draughtNumber > 0) {
            //通过风机编号查询风机状态百分比 返回给前端20条数据
            List<TbState> bydraughtNumber = baseService.findBydraughtNumberStateList(draughtNumber);
